...
2.  **Questioning Phase:** Ask a series of questions to gather details for the `spec.md` using the `AskUserQuestion` tool. You must batch up to 4 related questions in a single tool call to streamline the process. Tailor questions based on the track type (Feature or Other).
    ...
    *   **General Guidelines:**
        ...
        *   **2. Formulate the Question:** Use the `AskUserQuestion` tool: Adhere to the following:
            - **question:** "..."
            - **multiSelect:** (Required for additive choice) Set to `true` for multi-select (additive) or `false` for single-choice (exclusive).
            - **options:** Provide 2-4 options, each with a `label` and `description`. Note that "Other" is automatically added.

        *   **3. Interaction Flow:**
            *   Wait for the user's response after each `AskUserQuestion` tool call.
            *   If the user selects "Other", use a subsequent `AskUserQuestion` tool call to get their input if necessary.
...
2.  **Recommendation Loop:**
    -   **If relevant missing skills are found:**
        -   **Ask:** "Would you like to install these skills now?" using the `AskUserQuestion` tool (do not repeat in chat):
